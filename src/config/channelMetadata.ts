...
export const CHANNEL_METADATA: ChannelMetadata[] = [
  ...
  },
  {
    ...
    color: "bg-pink-500",
    catalogColor: "bg-gradient-to-br from-purple-500 to-pink-500",
    category: "business",
    catalogId: "instagram",
  ...
  },
  {
    key: "email",
    label: "Email",
    catalogName: "Email (SMTP / IMAP)",
    manageLabel: "Email (SMTP/IMAP)",
    description:
...
export const CHANNEL_CATALOG_ITEMS: ChannelCatalogItem[] = CHANNEL_METADATA
  ...
  .map((channel) => ({
    ...
    icon: channel.icon,
    color: channel.catalogColor ?? channel.color ?? "bg-indigo-500",
...
>((config, channel) => {
  ...
  config[channel.key] = {
    ...
    icon: channel.icon,
    color: channel.key === "messenger" ? "bg-indigo-600" : channel.color,
