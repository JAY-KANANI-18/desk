...
import { calendarEventClass, calendarStatusLabel, formatDateKey, formatTime } from "./utils";
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-4">
          <div>
            ...
            <p className="text-xs text-slate-500">Scheduled broadcasts by send date</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center overflow-hidden rounded-lg border border-slate-200">
              <button
                ...
                className="border-r border-slate-200 p-2 transition hover:bg-slate-50"
        ...
        <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-50">
          {CALENDAR_WEEK_DAYS.map((day) => (
            <div key={day} className="px-2 py-2 text-xs font-medium text-slate-500">
              {day}
            </div>
          ))}
        </div>
                ...
                <div className="space-y-1">
                  ...
                  {overflow > 0 && dayEvents[3] && (
                    <button
                      ...
                      className="w-full rounded-md px-2 py-1 text-left text-xs text-slate-500 transition hover:bg-slate-100"
                    >
                      +{overflow} more
                    </button>
                  )}
