...
interface TemplateModalProps {
  open: boolean;
  onClose: () => void;
  onUse: (renderedText: string, template: Template) => void;
...
const TEMPLATES: Template[] = [
  {
    id: '1', name: 'Customer Welcome', category: 'UTILITY', language: 'English', type: 'text',
    header: 'Welcome to {{brand_name}}!',
    body: "Hi {{contact_name}}, thanks for reaching out. We're here to help you with anything you need. Your reference ID is {{conversation_id}}.",
    footer: 'Reply STOP to unsubscribe',
    buttons: [
      { kind: 'quick_reply', label: 'Talk to Agent' },
      { kind: 'quick_reply', label: 'View FAQs' },
    ],
    variables: [
      { key: 'brand_name', label: 'Brand Name', defaultValue: 'Acme Support' },
      { key: 'contact_name', label: 'Contact Name' },
      { key: 'conversation_id', label: 'Conversation ID' },
    ],
  },
  {
    id: '2', name: 'Order Shipped', category: 'UTILITY', language: 'English', type: 'image',
    mediaUrl: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&q=80',
    body: "Great news, {{contact_name}}! Order #{{order_id}} has been shipped and will arrive by {{delivery_date}}.",
    footer: 'Track your package anytime',
    buttons: [
      { kind: 'url', label: 'Track Order', url: 'https://track.example.com/{{order_id}}' },
      { kind: 'quick_reply', label: 'Need Help?' },
    ],
    variables: [
      { key: 'contact_name', label: 'Contact Name' },
      { key: 'order_id', label: 'Order ID' },
      { key: 'delivery_date', label: 'Delivery Date', defaultValue: 'tomorrow' },
    ],
  },
  {
    id: '3', name: 'Product Demo Video', category: 'MARKETING', language: 'English', type: 'video',
    mediaUrl: 'https://images.unsplash.com/photo-1611532736597-de2d4265fba3?w=400&q=80',
    body: "{{contact_name}}, check out our latest product demo! Discover how {{product_name}} can transform your workflow.",
    footer: 'Limited time offer ends {{expiry}}',
    buttons: [
      { kind: 'url', label: 'Watch Full Demo', url: 'https://demo.example.com' },
      { kind: 'url', label: 'Buy Now', url: 'https://shop.example.com' },
    ],
    variables: [
      { key: 'contact_name', label: 'Contact Name' },
      { key: 'product_name', label: 'Product Name', defaultValue: 'Pro Suite' },
      { key: 'expiry', label: 'Expiry Date', defaultValue: 'Dec 31' },
    ],
  },
  {
    id: '4', name: 'Invoice Ready', category: 'UTILITY', language: 'English', type: 'document',
    header: 'Invoice #{{invoice_number}}',
    body: "Hi {{contact_name}}, your invoice for {{amount}} is ready. Please review and complete payment by {{due_date}}.",
    footer: 'Questions? Reply to this message',
    buttons: [
      { kind: 'url', label: 'View Invoice', url: 'https://pay.example.com/{{invoice_number}}' },
      { kind: 'phone', label: 'Call Billing', phone: '+1-800-555-0100' },
    ],
    variables: [
      { key: 'contact_name', label: 'Contact Name' },
  ...
  {
    id: '5', name: 'Flash Sale Carousel', category: 'MARKETING', language: 'English', type: 'carousel',
    body: "{{contact_name}}, our flash sale is LIVE! Swipe to explore top deals",
    cards: [
      {
        image: 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=300&q=80',
        title: 'Premium Watch', body: '50% off — only today!',
        buttons: [{ kind: 'url', label: 'Shop Now', url: '#' }, { kind: 'quick_reply', label: 'Save Deal' }],
      },
      {
        image: 'https://images.unsplash.com/photo-1585386959984-a4155224a1ad?w=300&q=80',
        title: 'Skincare Bundle', body: 'Buy 2 get 1 free',
        buttons: [{ kind: 'url', label: 'Shop Now', url: '#' }, { kind: 'quick_reply', label: 'Save Deal' }],
      },
      {
        image: 'https://images.unsplash.com/photo-1491553895911-0055eca6402d?w=300&q=80',
        title: 'Running Shoes', body: 'Starting at $49',
        buttons: [{ kind: 'url', label: 'Shop Now', url: '#' }, { kind: 'quick_reply', label: 'Save Deal' }],
      },
    ],
    variables: [{ key: 'contact_name', label: 'Contact Name' }],
  },
  {
    id: '6', name: 'Limited Time Offer', category: 'MARKETING', language: 'English', type: 'limited_time',
    header: 'Exclusive Deal for You',
    body: "{{contact_name}}, get {{discount}}% off your next purchase! Use code below at checkout.",
    footer: 'Offer valid while stocks last',
    expiryLabel: 'Offer expires in',
    expiryTime: '23:47:30',
    couponCode: '{{coupon_code}}',
    buttons: [
      { kind: 'copy_code', label: 'Copy Offer Code', code: '{{coupon_code}}' },
      { kind: 'url', label: 'Shop Now', url: 'https://shop.example.com' },
    ],
    variables: [
      { key: 'contact_name', label: 'Contact Name' },
      { key: 'discount', label: 'Discount %', defaultValue: '30' },
      { key: 'coupon_code', label: 'Coupon Code', defaultValue: 'SAVE30' },
    ],
  },
  {
    id: '7', name: 'Store Location', category: 'UTILITY', language: 'English', type: 'location',
    header: 'Find Our Nearest Store',
    body: "Hi {{contact_name}}, here's the location of our nearest store. We're open {{hours}}.",
    footer: 'Free parking available',
    buttons: [{ kind: 'url', label: 'Get Directions', url: 'https://maps.example.com' }],
    variables: [
      { key: 'contact_name', label: 'Contact Name' },
      { key: 'hours', label: 'Opening Hours', defaultValue: 'Mon–Sat 9am–8pm' },
    ],
  },
  {
    id: '8', name: 'OTP Verification', category: 'AUTHENTICATION', language: 'English', type: 'text',
    body: "{{otp_code}} is your verification code. For your security, do not share this code. Valid for {{validity}} minutes.",
    buttons: [{ kind: 'copy_code', label: 'Copy Code', code: '{{otp_code}}' }],
    variables: [
      { key: 'otp_code', label: 'OTP Code', defaultValue: '123456' },
      { key: 'validity', label: 'Validity (mins)', defaultValue: '10' },
    ],
  },
];
      ...
      <div className="px-3 py-4 flex flex-col gap-1.5">
        ...
        {template.type !== 'carousel' && (
          <div className="max-w-[88%] bg-white rounded-[14px] rounded-bl-[4px] shadow-sm overflow-hidden">
            ...
            {/* Text header (not doc/image/video/location) */}
...
export function TemplateModal({ open, onClose, onUse, contextValues = {} }: TemplateModalProps) {
  ...
  useEffect(() => {
    if (open) {
      setQuery(''); setSelected(null); setVarValues({}); setStep('list'); setActiveTab('all');
  ...
  const filtered = TEMPLATES.filter(t => {
    ...
    return matchCat && (!q || t.name.toLowerCase().includes(q) || t.type.includes(q) || t.category.toLowerCase().includes(q));
  });
  ...
  const handleUse = () => {
    ...
    onUse(renderTpl(selected.body, varValues), selected);
    onClose();
  };
        ...
        <div className="flex items-center gap-3 px-5 py-3.5 border-b border-gray-100 bg-white flex-shrink-0">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-gray-400">
              {step === 'list' ? `${TEMPLATES.length} approved templates` : `${selected?.category} · ${selected?.language}`}
            </p>
          </div>
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            <div className="px-4 pt-3 pb-2.5 flex-shrink-0 space-y-2 border-b border-gray-50">
              <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2">
                ...
                {query && <button onClick={() => setQuery('')}><X size={11} className="text-gray-400" /></button>}
              </div>
              <div className="flex gap-1.5 overflow-x-auto pb-0.5">
                {(['all', ...cats] as const).map(c => (
                  <button key={c} onClick={() => setActiveTab(c as any)}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {filtered.map(t => (
                    <button key={t.id} onClick={() => pick(t)}
                      ...
                      style={{ ':hover': { borderColor: '#128c7e20' } } as any}
                    >
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                            <span className="text-[12.5px] font-semibold text-gray-800">{t.name}</span>
                            <span className={`text-[8.5px] font-bold border px-1.5 py-0.5 rounded-full uppercase tracking-wide ${CAT_COLOR[t.category]}`}>
                              {t.category}
                            </span>
                          </div>
                          <p className="text-[11px] text-gray-400 truncate leading-snug">{t.body.replace(/\{\{[^}]+\}\}/g, '…')}</p>
                          <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                            <span className="text-[10px] text-gray-500 capitalize bg-gray-100 px-1.5 py-0.5 rounded-md">{t.type}</span>
