...
export function AgentDetailPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [nextDetail, nextTools, nextSources, nextAnalytics] =
        await Promise.all([
  ...
  const activeVersion = useMemo(
    () =>
      detail?.versions.find(
        (version) => version.id === detail.agent.activeVersionId,
  ...
  if (loading) {
    return (
      <AiPageLayout
        title="AI Agent"
        eyebrow="Automation"
        description="Loading the latest agent configuration."
      >
    ...
    return (
      ...
      >
        <div className="p-6 text-sm text-slate-500">
          This agent may have been archived or you may not have access.
        </div>
      </AiPageLayout>
    );
  }
  ...
  return (
    <AiPageLayout
      ...
      description={
        ...
        "Configure, test, and supervise this AI agent."
      }
      actions={
        <>
          <BackButton
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden lg:grid lg:grid-cols-[260px_1fr]">
        ...
        <main className="min-h-0 overflow-auto p-4 sm:p-6">
          ...
          {activeTab === "behavior" ? (
            <BehaviorTab agentId={agentId} version={draft} onSaved={load} />
          ) : null}
          {activeTab === "knowledge" ? (
            <KnowledgeTab sources={sources} onChanged={load} />
          ) : null}
          {activeTab === "tools" ? (
            <ToolsTab
              agentId={agentId}
          ...
          {activeTab === "guardrails" ? (
            <GuardrailsTab agentId={agentId} version={draft} onSaved={load} />
          ) : null}
          {activeTab === "versions" ? (
            <VersionsTab agentId={agentId} detail={detail} onChanged={load} />
          ) : null}
          {activeTab === "playground" ? (
            <PlaygroundTab agentId={agentId} />
          ) : null}
          {activeTab === "analytics" ? (
    ...
    <div className="space-y-5">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          detail={`Version ${activeVersion?.version || "draft"}`}
          icon={<Gauge size={18} />}
        />
        <MetricTile
          label="Handled"
        ...
        <MetricTile
          label="Resolution"
          value={`${summary?.runs ? Math.round(((summary.completed || 0) / summary.runs) * 100) : 0}%`}
          detail="Completed without failure"
          icon={<Check size={18} />}
        />
        <MetricTile
          label="Avg response"
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              The agent replies only where its published version allows it.
            </p>
          </div>
          <StatusBadge status={detail.agent.status} />
        </div>
        <div className="mt-4">
          <ChannelPills
            channels={
              activeVersion?.channelAllowlist ||
              detail.agent.activeChannels ||
              []
            }
          />
        </div>
    ...
    <div className="max-w-4xl rounded-lg border border-slate-200 bg-white p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Set the agent identity, tone, escalation posture, and office hours.
          </p>
        </div>
        <Button
          type="button"
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <Field
          label="Name"
          value={draft.name}
          onChange={(name) => setDraft((state) => ({ ...state, name }))}
        />
        <Field
          label="Tone"
          value={draft.tone}
          onChange={(tone) => setDraft((state) => ({ ...state, tone }))}
        />
        <Field
          label="Language"
          value={draft.defaultLanguage}
          onChange={(defaultLanguage) =>
            setDraft((state) => ({ ...state, defaultLanguage }))
          }
        />
        <TextareaInput
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Upload files, crawl websites, or import FAQs and catalogs.
        </p>
        <div className="mt-5 space-y-3">
          ...
          <Field
            label={
              sourceType === "website" ? "Website URL" : "Source URI or note"
            }
            value={uri}
            onChange={setUri}
          />
          {sourceType === "file" ? (
            <div className="rounded-md border border-dashed border-slate-300 p-4 text-center text-sm text-slate-500">
              ...
              <p className="mt-1 text-xs">
                File storage upload plugs into the existing media/files flow.
              </p>
            </div>
          ) : null}
          <Button
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Indexing progress, errors, sync state, and re-sync actions live
              here.
            </p>
          </div>
          <SearchInput
    ...
    <section className="max-w-4xl rounded-lg border border-slate-200 bg-white p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Only enabled tools can be invoked by this agent.
          </p>
        </div>
        <Button type="button" variant="dark" onClick={save}>
          Save
        </Button>
      </div>
          ...
          return (
            <Button
              ...
              onClick={() =>
                setAllowed((state) =>
                  checked
                    ? state.filter((item) => item !== tool.name)
                    : [...state, tool.name],
                )
              }
            >
...
function GuardrailsTab({
  ...
  const [config, setConfig] = useState({
    confidenceThreshold: Number(
      version.runtimeConfig?.confidenceThreshold ?? 0.65,
    ),
    ...
    <section className="max-w-3xl rounded-lg border border-slate-200 bg-white p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Controls that keep the agent grounded and escalation-friendly.
          </p>
        </div>
        <Button type="button" variant="dark" onClick={save}>
          Save
      ...
      <div className="mt-5 space-y-4">
        <Range
          ...
          onChange={(confidenceThreshold) =>
            setConfig((state) => ({ ...state, confidenceThreshold }))
          }
        />
        <Range
          label="Max auto replies before handoff"
          value={config.maxAutoReplies}
          min={1}
          max={12}
          step={1}
          onChange={(maxAutoReplies) =>
            setConfig((state) => ({ ...state, maxAutoReplies }))
          }
        />
        {[
          ...
          return (
            <div
              key={key}
              className="flex w-full items-center justify-between gap-3 rounded-md border border-slate-200 p-3 text-sm font-semibold text-slate-800"
            >
              {label}
              <ToggleSwitch
                checked={checked}
                aria-label={label}
                onChange={() =>
                  setConfig((state) => ({ ...state, [key]: !checked }))
                }
              />
            </div>
          );
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white">
      <div className="border-b border-slate-100 p-4">
        ...
        <p className="text-sm text-slate-500">
          Draft, published, and rollback history.
        </p>
      </div>
      <div className="divide-y divide-slate-100">
        {detail.versions.map((version) => (
          <div
            key={version.id}
            className="grid gap-3 p-4 md:grid-cols-[1fr_130px_160px_120px] md:items-center"
          >
            <div>
              ...
              <p className="text-sm text-slate-500">
                {version.tone} - {version.defaultLanguage}
              </p>
            </div>
...
function PlaygroundTab({ agentId }: { agentId: string }) {
  ...
  const run = async () => {
    ...
    setLoading(true);
    try {
      const nextResult = await aiAgentsApi.sandboxRun(agentId, {
    ...
    <div className="grid min-h-[620px] gap-4 xl:grid-cols-[minmax(340px,0.9fr)_minmax(420px,1.1fr)]">
      <section className="flex min-h-0 flex-col rounded-lg border border-slate-200 bg-white">
        <div className="border-b border-slate-100 p-4">
          ...
          <p className="text-sm text-slate-500">
            Sandbox runs never send real messages.
          </p>
        </div>
        <div className="flex-1 space-y-4 overflow-auto bg-slate-50 p-4">
          <div className="max-w-[80%] rounded-lg rounded-bl-sm bg-white p-3 text-sm text-slate-800 shadow-sm">
            {message}
          </div>
          {result?.reply ? (
            <div className="ml-auto max-w-[80%] rounded-lg rounded-br-sm bg-slate-950 p-3 text-sm text-white">
              {result.reply}
            </div>
          ) : null}
      ...
      <section className="min-h-0 overflow-auto rounded-lg border border-slate-200 bg-white">
        <div className="border-b border-slate-100 p-4">
          ...
          <p className="text-sm text-slate-500">
            Intent, confidence, tools, memory, prompt tokens, and final status.
          </p>
        </div>
        <div className="space-y-4 p-4">
          <DebugRow
            label="Detected intent"
            value={result?.decision?.intent || "Run a simulation"}
          />
          <DebugRow
          ...
          <DebugRow
            ...
            value={result?.decision?.responseStrategy || "Waiting"}
          />
          <DebugBlock
            title="Tools planned"
            data={result?.decision?.tools || []}
          />
          <DebugBlock title="Actions executed" data={result?.actions || []} />
          <DebugBlock
            title="Memory updates"
            data={result?.decision?.memoryUpdates || []}
          />
          <DebugBlock
            title="Run messages and tokens"
            data={runDetail?.messages || []}
          />
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricTile
          label="Handoff rate"
          value={`${summary?.runs ? Math.round(((summary.escalated || 0) / summary.runs) * 100) : 0}%`}
        />
        <MetricTile
