...
### 2026-03-01 — `Cannot use 'import.meta' outside a module` (recurring)

**Status:** Fixed
**Symptoms:** Build error in `src/lib/supabase.ts` and `src/context/AuthContext.tsx` (transitive). Sandpack bundler throws `SyntaxError: Cannot use 'import.meta' outside a module`.
**Root Cause:** Sandpack uses a CodeSandbox-style bundler that doesn't support Vite's `import.meta.env`. Previous fixes tried guarding with `typeof import.meta`, which also fails.
**Solution:** Replace `import.meta.env.VITE_*` with `process.env.VITE_*`. Sandpack's bundler injects `process.env` correctly.
**Prevention:** In this Sandpack environment, always use `process.env` instead of `import.meta.env` for environment variables.
